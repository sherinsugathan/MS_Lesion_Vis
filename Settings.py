...
class Settings:
  def __init__(self, lh_pial_enabled, rh_pial_enabled, lh_white_enabled, rh_white_enabled, lh_pial_transparency, rh_pial_transparency, lh_white_transparency, rh_white_trasparency, depthPeelingEnabled, visType):
    self.lh_pial_enabled = lh_pial_enabled
    self.rh_pial_enabled = rh_pial_enabled
    self.lh_white_enabled = lh_white_enabled
    self.rh_white_enabled = rh_white_enabled
  ...
  def __str__(self):
    return "Lh Pial Enabled:" + str(self.lh_pial_enabled) + ", Rh Pial Enabled:" + str(self.rh_pial_enabled) +  ", Lh White Enabled:" + str(self.lh_white_enabled) + \
      ", Rh White Enabled:" + str(self.rh_white_enabled) + ", Lh Pial Transparency:" + str(self.lh_pial_transparency) + ", Rh Pial Transparency:" + str(self.rh_pial_transparency) + \
...
def getSettings(visType):
  ...
  if(visType == VisType.DEFAULT): # Default View
    setting = Settings(True, True, True, True, 1.0, 1.0, 1.0, 1.0, True, VisType.DEFAULT)
  elif(visType == VisType.TRANSPARENT): # Transparent view with few surfaces.
    setting = Settings(True, True, False, False, 0.5, 0.5, 0.5, 0.5, True, VisType.TRANSPARENT)
  elif(visType == VisType.RAW_LESION_INTENSITY): # Raw intensity color map on lesions.
    setting = Settings(True, True, False, False, 0.5, 0.5, 0.5, 0.5, True, VisType.RAW_LESION_INTENSITY)
  elif(visType == VisType.RAW_LESION_CONTRAST): # Continuous color map of lesion contrast.
    setting = Settings(True, True, True, True, 0.5, 0.5, 0.5, 0.5, True, VisType.RAW_LESION_CONTRAST)
  elif(visType == VisType.LESION_CLASS_VIEW): # Class view of lesions based on intensity contrast w.r.t normal white matter.
    setting = Settings(True, True, True, True, 0.5, 0.5, 0.5, 0.5, True, VisType.LESION_CLASS_VIEW)
  elif(visType == VisType.SURFACE_MAPPING): # Lesions projected to surface view.
    setting = Settings(True, True, True, True, 0.5, 0.5, 0.5, 0.5, True, VisType.SURFACE_MAPPING)
  return setting
