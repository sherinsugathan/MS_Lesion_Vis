...
for pElementIndex in range(len(listOfSegmentedParcellationVerticesRh)): # for every parcellation
    ...
    for jsonElementIndex in (range(1,numberOfLesionElements+1)): # for every lesion
        for p in structureInfo[str(jsonElementIndex)]:
            ...
            if impactRh.size>0 and listOfSegmentedParcellationVerticesRh[pElementIndex].size>0:
                intersectionList = np.intersect1d(impactRh, listOfSegmentedParcellationVerticesRh[pElementIndex])
    ...
    properties['PercentageAffected'] = (len(uniqueAffectedPoints)/listOfSegmentedParcellationVerticesRh[pElementIndex].size)*100
    #properties['vertexIdsRh'] = 1
    #properties['NumberOfPixels'] = 2
    data[pElementIndex]=[]
    data[pElementIndex].append(properties)
...
for pElementIndex in range(len(listOfSegmentedParcellationVerticesLh)): # for every parcellation
    ...
    for jsonElementIndex in (range(1,numberOfLesionElements+1)): # for every lesion
        for p in structureInfo[str(jsonElementIndex)]:
            ...
            #print("Length of impactRh", len(impactRh))
            #print("Length of parcellation verts", len(listOfSegmentedParcellationVerticesRh[pElementIndex]))
            if impactLh.size>0 and listOfSegmentedParcellationVerticesLh[pElementIndex].size>0:
                intersectionList = np.intersect1d(impactLh, listOfSegmentedParcellationVerticesLh[pElementIndex])
                if(intersectionList.size>0):
                    lesionImpactData[jsonElementIndex] = intersectionList.tolist()
                    #print("Intersection")
                #else:
    ...
    for key in lesionImpactData: 
        affectedPoints.extend(lesionImpactData[key])
    uniqueAffectedPoints = np.unique(affectedPoints) 
    properties['PercentageAffected'] = (len(uniqueAffectedPoints)/listOfSegmentedParcellationVerticesLh[pElementIndex].size)*100
    #properties['vertexIdsRh'] = 1
    #properties['NumberOfPixels'] = 2
    data[pElementIndex]=[]
    data[pElementIndex].append(properties)
