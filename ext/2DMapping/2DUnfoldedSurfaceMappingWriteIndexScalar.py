...
for subjectString in listOfSubjects:
        for hm in hemisphere:
                for labelF in labelFiles:
                        surfaceFile = "D:\\OneDrive - University of Bergen\\Datasets\\MS_SegmentationChallengeDataset\\"+subjectString+"\\surfaces\\"+hm+".pial.obj"
                        labelFile = "D:\\OneDrive - University of Bergen\\Datasets\\MS_SegmentationChallengeDataset\\"+subjectString+"\\surfaces\\"+hm+ labelF
                        ...
                        intIndexArray = vtk.vtkIntArray()
                        intIndexArray.SetName("OriginalVertexIds")

                        for index in range(numberOfPoints):
                            intIndexArray.InsertNextValue(index)

                        surface_mapper.GetInput().GetPointData().SetScalars(intIndexArray)
